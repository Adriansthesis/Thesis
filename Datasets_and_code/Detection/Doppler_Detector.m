function [Detections] = Doppler_Detector(time_step,time_overlap,range_step,range_overlap,Pfa, Window_Size, Gaurd_Cells,Np,PRF,Target_region,mean_threshold)
...
for i = ceil(time_step_s/2):round(time_overlap_s):target_size(1)-ceil(time_step_s/2)
	for k = ceil(range_step/2):round(range_overlap):target_size(2)- ceil(range_step/2)
		...
		for l = 1:target_doppler_size(2)
			Threshold(:,l) = OS_CFAR(Pfa, Window_Size, Gaurd_Cells, Window_doppler(:,l),round(2*Window_Size*0.99));	
			%Threshold(l,:) = CA_CFAR(Pfa, Window_Size, Gaurd_Cells, Window_doppler(l,:));	
		end
		...
		if mean(Detections_doppler(:)) > mean_threshold*0
