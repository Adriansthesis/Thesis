function [Detections] = Doppler_Detector_Rec(time_step,time_overlap,range_step,range_overlap,Pfa, Window_Size_X, Gaurd_Cells_X, Window_Size_Y, Gaurd_Cells_Y,Np,PRF,Target_region)
...
for i = ceil(time_step_s/2):round(time_overlap_s):target_size(1)-ceil(time_step_s/2)
	for k = ceil(range_step/2):round(range_overlap):target_size(2)- ceil(range_step/2)
        ...
        Threshold = OS_CFAR_Rec(Pfa, Window_Size_X, Gaurd_Cells_X, Window_Size_Y, Gaurd_Cells_Y, Window_doppler, round(N*0.8));

			
		
	
		Detections_doppler = (Window_doppler-Threshold)>0;
		Detections_doppler(1:Window_Size_X+Gaurd_Cells_X,:) = 0;
		Detections_doppler(end - (Window_Size_X+Gaurd_Cells_X): end,:) = 0;
		Detections_doppler(:,1:Window_Size_Y+Gaurd_Cells_Y) = 0;
		Detections_doppler(:,end - (Window_Size_Y+Gaurd_Cells_Y): end) = 0;
