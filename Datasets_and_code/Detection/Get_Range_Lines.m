...
function [sif,range_axis,time_axis,numPulses] = Get_Range_Lines(wavFile,BW)
% Yunus
...
Tp = 5e-3; % (s) minimum pulse length
...
trig = 10*Y(:,1); % the trigger signal is in the first channel
...
sif2 = convn(sif,mti_filter,'same');

range_axis = dataRange;
time_axis = (0:numPulses-1)*Tp*2;
% % display the RTI
figure;
imagesc(dataRange,(0:numPulses-1)*Tp*2,20*log10(abs(sif2)));
ylabel('Time (s)');
xlabel('Range (m)');
title('RTI with 2-pulse cancel');
colormap(jet(256));
colorbar;
axis xy;
